...
const page = () => {
    // returns session as server response
    const {data : session} = useSession();
    const router = useRouter();
    
    // If session is true render this else render something else
    useEffect(() => {
    ...
    // Sends request to server then server AuthProvider authenticates and then returns session as response
                            ...
                            <div className="p-8">
                                <div className="space-y-4">
                                    {/* <img src="https://www.svgrepo.com/show/475643/dribbble-color.svg" loading="lazy" */}
                                        {/* className="w-10" /> */}
                                        <h2 className="text-center mb-8 text-2xl text-cyan-900 dark:text-white font-bold">Log-In To Create Your Account.
                                        </h2>
                                </div>
                                <div className="mt-10 grid space-y-4">
                                    <button
                                        onClick={() => signIn('google')}
                                        className="cursor-pointer group h-12 px-6 border-2 border-gray-300 rounded-full transition duration-300 hover:border-blue-400 focus:bg-blue-50 active:bg-blue-100">
